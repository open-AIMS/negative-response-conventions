...
R/cases.R        the four alga datasets, prepared for the same conventions
R/estimates.R    endpoint extraction, model weights, diagnostics
analysis/        the per-unit runners, build_priors.R, collate.R, report.qmd
priors/          one prior per cell and convention, fixed across realisations
results/         one .rds per unit; the collated tables the report reads
results_cases/   the case-study units
hpc/             container lock, job scripts, deploy and fetch
```
